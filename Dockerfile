...
# (both unreferenced duplicate sock-mount templates) were removed too.
# docker.sock is now reachable ONLY through the existing in-app consent
# flow in app.js (/docker/confirm) — unchanged, still requires an explicit
# user click plus a manual Stop -> Start in SoloHost.
